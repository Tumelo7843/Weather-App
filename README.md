...
## If you prefer hosting frontend on GitHub Pages
- Keep the backend on Render. Set CORS is enabled in `server.js` so the frontend can call the Render URL (you'll need to update `public/script.js` fetch base URLs to point to the Render service URL).
...
If you want, I can:
- Prepare the `git rm --cached .env` and history cleanup commands.
